...
def expm_(h):
    """A faster ersatz for matrix exp, it has second order
    accuracy when h is small. exp(h)~(I+h/2)/(I-h/2)
    ...
    ----
        exp(h): np.ndarray
            Unitary transformation generated by h
    """
    I = np.eye(*h.shape)
    U = np.linalg.solve(I - h/2, I + h/2)
    return U
