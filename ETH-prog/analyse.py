...
def draw_diff_matrix(Hf, arg_tpl, _optimize, arg_opt, arg_clt):
    ...
    xlabel("site")
    ylabel("s")
    title(r'Variance of $\sigma_x$ after optimization (L={n}, J={J}, h={h}, g(\mu,\delta)={g}, depth={D})'.format(D=D, **arg_tpl))
    ...
    for D in args.depth:
        ...
        for w in args.window:
            loadData(
                ...
                {"n":6, "J": 1, "h":0.2, "g": (0, w)},
                ol.minimize_local,
                {'D':D, 'L':6, 'n':args.niter, 'rel':args.rel},
                {'rs': rs, 'rs_rot': rs_rot, 'ns': 5, 'nit': 15},
                pre="_D={}".format(D)
                )
