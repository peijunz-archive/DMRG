...
def Measure(n, delta=1/2, g=1/2, ns=11, nit=10, rs=None):
    ...
    for i, s in enumerate(np.linspace(0, 4, ns)):
        print("Entropy S", s)
        for j in range(nit):
            H4=Hamilton_XX(n, delta, g, rs)
    ...
    print(name, 'saved!')
    return a
def draw(n, delta, g, rs=None):
    name = fname(n, delta, g, rs)
    try:
        a = np.load("data/"+name+'.npy')
    except FileNotFoundError:
        print("Measure", n, delta, g)
        a = Measure(n, delta, g, rs=rs)
    ...
    s=std(dif, axis=1)
    for i in range(11):
        errorbar(arange(-3, 4), m[i], s[i], label="{:.1f}".format(abs(ms[i])), capsize=1.5)
    grid()
    title('Diff between rho for random H(%d, %.2lf, %.2lf)'%(n, delta, g));
...
if __name__ == "__main__":
    ...
    for g in [0, 0.1, 0.2, 0.5, 0.7, 1, 2, 3, 4,6,8,12]:
        draw(4, 1/2, g, rs=rs)
